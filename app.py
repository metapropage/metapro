...
def authenticate(username, password):
    if username == USERNAME and password == PASSWORD:
        ...
        if "persistent_login_time" not in st.session_state or st.session_state.persistent_login_time is None:
            st.session_state.login_time = datetime.now()
            st.session_state.persistent_login_time = st.session_state.login_time
        else:
            st.session_state.login_time = st.session_state.persistent_login_time
    else:
...
if st.session_state.authenticated:
    ...
    if st.session_state.login_time:
        ...
        remaining_time = timedelta(minutes=30) - (datetime.now() - st.session_state.login_time)
        if remaining_time.total_seconds() <= 0:
            st.session_state.authenticated = False
            st.session_state.role = None
            st.session_state.login_time = None
            st.session_state.persistent_login_time = None
            st.error("Trial period has expired. Please subscribe by contacting the admin.")
        else:
            # Display the countdown timer
            minutes, seconds = divmod(remaining_time.total_seconds(), 60)
            timer = f"{int(minutes):02d}:{int(seconds):02d}"
            st.sidebar.markdown(f"**Time remaining:** {timer}")

            st.experimental_set_query_params(refresh=1)  # Auto-refresh every second to update the timer
        ...
        st.error("Session time not set. Please login again.")
