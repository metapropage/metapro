...
def main():
    ...
    if st.session_state['license_validated']:
        ...
        if current_date > expiration_date:
            ...
            st.success(f"License valid. You have {days_remaining} days remaining.") License valid. Maximum upload limit is 45 files at once, with unlimited uploads allowed each day
        ...
        # Upload image files
        uploaded_files = st.file_uploader('Upload Images (Only JPG and JPEG Supported, Maximum 45 Files)', accept_multiple_files=True)
        ...
        if uploaded_files:
            ...
            # Check if the total files exceed 45
            if total_files_to_upload > 45:
                st.error("You can only upload up to 45 files at a time.")
                return
