...
if __name__ == '__main__':
    main()
In this updated function, the following changes were made:

The file upload limit is set to 45.
If the user uploads more than 45 files, a warning is displayed, and only the first 45 files are processed.
The rest of the code handles processing these files, embedding metadata, and uploading them to Google Drive as before.
file yang di upload per file selesai dulu dari yang paling awal, baru upload file kedua, dan jika sudah selesai 45 upload ,proses upload akan berhenti otomatis dan bisa di proses 45 file tersebut
ChatGPT
To process each file sequentially and stop after 45 files have been processed, you can modify the main() function accordingly. Here is the updated code:
...
The process will stop after 45 files have been processed, and a zip file will be created containing these processed files.
The zip file will then be uploaded to Google Drive, and the link will be provided to the user.
masih upload bareng, buat each per file uploadnya dari file pertama
ChatGPT
To ensure that each file is processed and uploaded sequentially, and stop after 45 files, the code needs to be adjusted to handle each file individually. Here is the updated code:
