...
def generate_metadata(model, img):
    ...
    tags = model.generate_content(["As the helpful Digital Asset Metadata Manager, analyze the following image and generate search engine optimized keywords for stock photography. Generate up to 45 keywords in English that are relevant to the image (each keyword must be one word, separated by commas). Ensure each keyword is a single word, separated by commas.", img])
